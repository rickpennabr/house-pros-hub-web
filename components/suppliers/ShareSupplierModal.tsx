...
export default function ShareSupplierModal({
  isOpen,
  onClose,
  supplierName,
  branchName,
  ...
  const handleShare = (platform: string) => {
    ...
    const displayName = branchName ? `${supplierName} - ${branchName}` : supplierName;
    const encodedText = encodeURIComponent(`Check out ${displayName} - ${supplierType}`);
    ...
    const shareUrls: Record<string, string> = {
      ...
      email: `mailto:?subject=${encodeURIComponent(displayName)}&body=${encodedText}%20${encodedUrl}`,
    };
    ...
    >
        ...
        <div className="p-4 border-b-2 border-black">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-lg font-bold text-black truncate">{supplierName}</h3>
              {branchName && <p className="text-sm text-gray-600 truncate">{branchName}</p>}
              <p className="text-sm text-gray-600 truncate">{supplierType}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-b-2 border-black">
          <div className="bg-yellow-100 border-2 border-black rounded-lg p-3 flex items-center justify-between gap-2">
            <p className="text-sm font-medium text-black truncate flex-1">{supplierUrl}</p>
            <button
