...
export function AuthPageLayout({ 
  ...
  const resolvedTitle = title ?? t('tagline');
  return (
      ...
      <div className="w-full md:w-[90%] h-[calc(100vh-1rem)] md:max-h-[90vh] bg-white rounded-lg overflow-hidden border-2 border-black flex flex-col">
        <div className="relative grid grid-cols-1 md:grid-cols-2 flex-1 min-h-0 overflow-hidden">
          ...
          <div className="hidden md:flex flex-col bg-white px-6 py-12 border-r-2 border-black h-full relative z-10">
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-semibold mb-6 text-center text-black">
              {resolvedTitle}
            </h1>
          ...
          {/* Right Side - Form */}
          <div className="flex flex-col bg-white text-black transition-all duration-500 ease-in-out flex-1 min-h-0 overflow-y-auto px-3 md:px-12 pb-3 md:pb-12">
            {children}
          </div>
        </div>
      </div>
