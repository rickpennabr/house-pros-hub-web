...
const CATEGORY_OPTIONS = ['materials', 'labor', 'subcontractor', 'equipment', 'permits', 'other'] as const;
...
export default function CRMExpensesPage() {
  ...
  const fetchExpenses = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/crm/expenses?${params}`, { credentials: 'include' });
      ...
      if (isFirst) { setExpenses([]); setTotal(0); }
    } finally {
      setIsLoading(false);
    }
  }, [page, sortBy, sortDir]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetch(`/api/crm/expenses/${editingId}`, { credentials: 'include' })
      .then((r) => r.json())
      .then((data) => {
        if (cancelled) return;
        setFormProjectId(data.projectId ?? '');
        setFormAmountDollars(data.amountCents != null ? (data.amountCents / 100).toFixed(2) : '');
        setFormCategory(data.category ?? 'other');
        setFormDate(data.expenseDate ? data.expenseDate.slice(0, 10) : '');
        setFormNote(data.note ?? '');
      })
      .catch(() => { if (!cancelled) setFormAmountDollars(''); });
  ...
  const handleDelete = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/crm/expenses/${id}`, { method: 'DELETE', credentials: 'include' });
  ...
  const handleSubmitAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setViewMode('table');
      setFormProjectId(''); setFormAmountDollars(''); setFormCategory('other'); setFormDate(''); setFormNote('');
      fetchExpenses();
    } catch (err) {
  ...
  const handleSubmitEdit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/crm/expenses/${editingId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const defaultDate = formDate || new Date().toISOString().slice(0, 10);
        ...
        <>
          <div className="flex flex-row items-center justify-between gap-4 pt-2 pb-2 md:pt-0 md:pb-0">
            <div className="min-w-0">
              ...
              <p className="text-sm text-gray-500 mt-0.5">Track expenses and optionally link to projects.</p>
            </div>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm">
            {isLoading && !hasLoadedOnce ? (
              <div className="p-12 text-center text-gray-500 text-sm">Loading expenses…</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full border-collapse" style={{ minWidth: 450 }}>
                  <thead>
                    <tr className="border-b-2 border-black bg-black text-white">
                      <th className="px-4 py-1.5 text-left border-r border-white whitespace-nowrap"><SortHeader label="Date" sortKey="expense_date" currentSort={sortBy} currentDir={sortDir} onSort={handleSort} /></th>
                      <th className="px-4 py-1.5 text-left border-r border-white whitespace-nowrap"><SortHeader label="Amount" sortKey="amount_cents" currentSort={sortBy} currentDir={sortDir} onSort={handleSort} /></th>
                      <th className="px-4 py-1.5 text-left border-r border-white whitespace-nowrap"><SortHeader label="Category" sortKey="category" currentSort={sortBy} currentDir={sortDir} onSort={handleSort} /></th>
                      <th className="px-4 py-1.5 text-left border-r border-white whitespace-nowrap">Note</th>
                      <th className="sticky right-0 z-10 py-1.5 text-center bg-black border-l-2 border-white w-[72px]">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {expenses.length === 0 ? (
                      <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-500 text-sm">No expenses found.</td></tr>
                    ) : (
                      expenses.map((row) => (
                        <tr
                          key={row.id}
                        ...
                        >
                          <td className="px-4 py-1.5 text-sm text-gray-600">{formatDate(row.expense_date)}</td>
                          <td className="px-4 py-1.5 text-sm text-gray-900 font-medium">{formatAmount(row.amount_cents)}</td>
                          <td className="px-4 py-1.5 text-sm text-gray-600">{row.category}</td>
                          <td className="px-4 py-1.5 text-sm text-gray-500 max-w-[200px] truncate">{row.note || '—'}</td>
          ...
          {!isLoading && total > 0 && <div className="mt-4"><Pagination currentPage={page} totalPages={totalPages} onPageChange={setPage} totalItems={total} itemsPerPage={PAGE_SIZE} itemLabel="expenses" /></div>}
        </>
      )}
      ...
      {(viewMode === 'add' || viewMode === 'edit') && (
        <div className="w-full md:max-w-xl md:mx-auto">
          <div className="py-2 md:mb-6 flex items-center justify-between gap-4">
            ...
            <span className="text-lg font-semibold text-gray-900">{viewMode === 'add' ? 'New Expense' : 'Edit Expense'}</span>
          </div>
          <form onSubmit={viewMode === 'add' ? handleSubmitAdd : handleSubmitEdit} className="space-y-4">
            ...
            <FormField label="Amount ($)" required>
              <Input type="number" step="0.01" min="0" value={formAmountDollars} onChange={(e) => setFormAmountDollars(e.target.value)} placeholder="0.00" required />
            </FormField>
            ...
            <FormField label="Date" required>
              <Input type="date" value={formDate || defaultDate} onChange={(e) => setFormDate(e.target.value)} required />
            </FormField>
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button type="submit" variant="primary" size="sm" disabled={saving}>{saving ? 'Saving…' : viewMode === 'add' ? 'Create Expense' : 'Save changes'}</Button>
            </div>
          </form>
        </div>
      )}
      ...
      {deleteConfirm && (
        <Modal isOpen onClose={() => !deleting && setDeleteConfirm(null)} showHeader={false} showCloseButton maxWidth="sm" preventCloseOnOverlayClick={deleting}>
          <div className="p-6 text-center">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2">Delete expense?</h2>
